...
class ParametricCurve(Curve, ABC):
    ...
    def find_u(
        ...
        Args:
            ...
            verbose (bool):
                Whether to print additional result information.
                Defaults to False.
...
class BSpline2D(ParametricCurve):
    ...
    def fit(
        ...
        """
        Fit a 2D spline to the given points.

        Does nothing but forward the inputs. mainly here for consistency with
        other classes.

        Args:
            points (np.ndarray): Input coordinates for fitting the curve. degree
            (int): Degree of the spline. Defaults to 3 (cubic spline). smoothing
            (float): Smoothing factor for the spline. Defaults to 0.0.

        Returns:
            BSpline2D: Fitted 2D spline.
        """
        return cls(points, degree, smoothing)
...
class Bezier(ParametricCurve):
    ...
    def spline(self) -> Tuple[np.ndarray, np.ndarray, int]:
        ...
        """
        return (self.knots, self.control_points, self.degree)
    ...
    def fit(
        ...
        # adjust number of control points to fit if endpoints are fixed
        if fix_endpoints:
            n_control_points -= 2
        ...
        def objective(flat_control_points: np.ndarray) -> np.ndarray:
            ...
            Returns:
                np.ndarray: shortest distances between data points and curve.
            """
        ...
        results = opt.minimize(objective, init_guess.ravel())
...
class SplevBezier(ParametricCurve):
    ...
    def fit(
        ...
        Args:
            points (np.ndarray):
                Input coordinates for fitting the curve.
...
class SplevCBezier(BSpline2D):
    ...
    def __init__(
        ...
        Args:
            ...
            points (np.ndarray, optional): Input coordinates for fitting the
            curve.

        Raises:
            ValueError: knot vector is not non-decreasing.

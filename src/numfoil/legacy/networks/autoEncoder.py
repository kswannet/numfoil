...
class AutoEncoder(nn.Module):
    def __init__(self, *, input_dim, h_dim, z_dim, actFun=nn.LeakyReLU(), dropRate=0):
        ...
        self.z_dim = z_dim              # dimension of the latent vector
        ...
        self.encoderLayers = nn.ModuleList(
            [nn.Linear(layer_dim[i], layer_dim[i+1]) for i in range(num_layers-1)]
        )
    ...
    def encode(self, h: torch.Tensor) -> torch.Tensor:
        ...
        """
        for layer in self.encoderLayers[:-1]:               # For each layer in encoder list, except for the last one
        ...
        return self.sigmoid(self.encoderLayers[-1](h))      # final layer and (optional) activation function
